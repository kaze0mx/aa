            ...
            for(int wsize=1024; wsize <= 1024; wsize+=512) {
                for(float penalty=0.01; penalty <= 0.65; penalty+=0.2) {
                    ...
                    for(int algo=0; algo < AA_ALG_MAX; algo++) {
                        if(algo==AA_ALG_VECTOR_11_FILL || algo==AA_ALG_VECTOR_DST_FILL)
                            continue;
                        if(!aa_convert(image, (AaAlgorithmId)algo, &font, &aaimage, fsize, wsize, DEFAULT_TRANSLATION, penalty, AA_PAL_FREE_64,DEFAULT_CANNY_SIGMA,DEFAULT_CANNY_HYS_MIN,DEFAULT_CANNY_HYS_MAX,DEFAULT_BLUR_PASS)) {
