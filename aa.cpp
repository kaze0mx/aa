...
#ifdef DEBUG
				printf("%c=%2.2f ",font->font_carmap[font->font_subset[ii]],err);
#endif				
				if(err<minerr) {
					minerr=err;
					bestind=mat_ind;
 ...
 *
 */
bool aa_convert(FIBITMAP* image, AaAlgorithmId algorithm, AaFont* font, AaImage* res, int lines, int working_height, int translation, float penalty, AaPaletteId palette_id, float canny_sigma, int canny_min, int canny_max, int blur_pass) {
	...
	bool clipart=is_clipart(final);
	if(algorithm >= AA_ALG_VECTOR_DST && algorithm <= AA_ALG_VECTOR_11_FILL && !clipart) {
		//edges
#ifdef DEBUG
		...
		if(blur_pass > 0) {
            float p=1.0/8;
            float blur[]= { p, p, p, p,0, p, p, p, p };
            final=convolute(final,blur,3,true);
            for(int i=0;i<blur_pass-1;i++)
                final=convolute(final,blur,3,false);
#ifdef DEBUG
            GenericWriter(final,"blurred.png",0);
#endif
        }
		
		final=threshold(final,THRESHOLD_PIC);
...
#ifdef DEBUG
		GenericWriter(final,"thinned.png",0);
#endif 
    ...
    if(algorithm >= AA_ALG_VECTOR_DST && algorithm <= AA_ALG_VECTOR_11_FILL) {
        ...
        if(clipart) {
			...
			resized=threshold(resized,THRESHOLD_CLIPART);
        }
        else {
            resized=normalize(resized);
			resized=threshold(resized,THRESHOLD_PIC);
        }
    }
    else {
        if(clipart)
    ...
    if(algorithm >= AA_ALG_VECTOR_DST && algorithm <= AA_ALG_VECTOR_11_FILL) {
        for(int y=0; y < height-PASY; y+=PASY) {
            for(int x=0; x < width; x+=PASX) {
                ...
                BYTE color=get_best_color(paletized,x,y);
